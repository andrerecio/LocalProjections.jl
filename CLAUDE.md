...
The entire package is **one file**, `src/LocalProjections.jl` (~1740 lines), with no `include`s; it is organized by banner comments in this order: formula-term types (`CumulTerm`, `LeadTerm`, `AnchorTerm`, `|` pipe) → `LocalProjection` types → data transforms → `lp()` → `stderror`/`IRFSummary` → IV block (`lpiv`, first-stage/weak-IV) → shared `coefpath`/`vcov`/`summarize` → plotting (RecipesBase) → `as_irf_result` bridge. Navigate by section, not by file.
