...
- **REG-2 (upstream inconsistency in Regress.jl, verified 2026-09-20 to ~15 digits against Stata 17):** `lp` and `lpiv` covariances follow different conventions for the same estimator. (a) *d.o.f. factor:* on OLS models `n/(n−k)` is applied to `HR1`, kernel HAC and EWC (OLS kernel HAC ≡ Stata `newey`); on IV models it is applied to `HR1` but **not** to kernel HAC or EWC (IV kernel HAC ≡ unadjusted `ivregress`/`ivreg2` default). So `lpiv` kernel-HAC standard errors are smaller than `lp` ones by `√(n/(n−k))` per horizon (~5% with `k ≈ 20`, `n ≈ 200`), and REG-1 means the caller cannot harmonize it (`dofadjust=true` on an IV model throws `MethodError: bread`). (b) *automatic bandwidth:* the OLS path gives the intercept kernel weight 0 in `Bartlett{NeweyWest}`/`Andrews` selection, the IV `aVar` override gives it weight 1, so numerically identical OLS/IV fits select different bandwidths (a per-sample reproducibility defect: a few % typically, occasionally much more; no effect on average coverage). **Deliberately not patched downstream** — a rescale in `LocalProjections.vcov` would double-count once upstream is fixed, and the upstream direction is undecided; drafts are in `upstream_issue_drafts.md` (Issues 3–4). Until then: use a fixed bandwidth (e.g. `Bartlett(6)`) when `lp` and `lpiv` bands must be comparable, and do not "fix" either convention silently. Conventions for cross-checks: `Bartlett(bw)` ≡ `ivreg2 bw(bw)` ≡ `newey lag(bw−1)`; automatic bandwidths cannot match Stata (pilot length `20·(T/100)^{2/9}` with a cap there vs `4·(T/100)^{2/9}` in CovarianceMatrices), so only fixed-bandwidth Stata numbers are reference values.
